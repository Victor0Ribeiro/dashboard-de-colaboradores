...
async function readEmployees() {
    ...
    sheet.eachRow((row, rowNumber) => {
        ...
        employees[id] = {
            ...
            Paciência: Number(row.getCell(4).value) || 0,
            Versatilidade: Number(row.getCell(5).value) || 0,
            Comunicação: Number(row.getCell(6).value) || 0
        };
    });
    return employees;
}

// Salva funcionários no Excel
...
// API para obter funcionários
app.get('/api/employees', async (req, res) => {
    const employees = await readEmployees();
    res.json(employees);
});

// API para salvar funcionários
